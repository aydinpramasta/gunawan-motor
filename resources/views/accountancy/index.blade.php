        ...
        <form class="d-flex flex-column flex-md-row" action="{{ route('accountancies.index') }}" method="get">
          <div class="form-group mr-3 mt-3 mt-md-0">
            <div class="custom-control custom-radio">
              <input class="custom-control-input" type="radio" id="type-income" name="type" value="income"
                {{ request()->input('type') === 'income' ? 'checked' : '' }}>
              <label for="type-income" class="custom-control-label">Pemasukan</label>
            </div>
            <div class="custom-control custom-radio">
              <input class="custom-control-input" type="radio" id="type-expense" name="type" value="expense"
                {{ request()->input('type') === 'expense' ? 'checked' : '' }}>
          ...
          <div class="form-group mr-3">
            <div class="custom-control custom-radio">
              <input class="custom-control-input custom-control-input-danger" type="radio" id="date-weekly"
                name="date" value="weekly" {{ request()->input('date') === 'weekly' ? 'checked' : '' }}>
              <label for="date-weekly" class="custom-control-label">Mingguan</label>
            </div>
            <div class="custom-control custom-radio">
              <input class="custom-control-input custom-control-input-danger" type="radio" id="date-monthly"
                name="date" value="monthly" {{ request()->input('date') === 'monthly' ? 'checked' : '' }}>
              <label for="date-monthly" class="custom-control-label">Bulanan</label>
            </div>
          </div>

          <a href="{{ route('accountancies.index') }}" class="btn btn-outline-warning mr-0 mr-md-2"
          ...
          <tbody>
            @foreach ($accountancies as $accountancy)
              <tr>
                ...
                <td>
                  <a href="{{ route('accountancies.edit', ['accountancy' => $accountancy->id]) }}"
                    class="m-1 btn btn-sm btn-warning">
                    <i class="fas fa-edit"></i>
                  </a>

                  <form action="{{ route('accountancies.destroy', ['accountancy' => $accountancy->id]) }}"
                    method="post">
                    @csrf
                    @method('delete')
